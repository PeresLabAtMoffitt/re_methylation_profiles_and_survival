...
print(paste0("outliers : ", outlier_sample))
betas_clean6 <- betas_clean6[, !colnames(betas_clean6) %in% c(outlier_sample)]
rm(beta, q2575, qq, flag, low, qr, up)
print(paste0("The dimension of the data is : ", dim(betas_clean6)))
```

```{r}
map_df <- betas_clean6 %>% as_tibble(rownames = "cpgs")

plot <- map_df %>% 
  pivot_longer(cols = - cpgs) %>% 
...
library(REMP)
citation("REMP")
# getLocations(IlluminaHumanMethylationEPICanno.ilm10b2.hg19)      # NOT RUN
# genomic_location_matrix <- getLocations(IlluminaHumanMethylationEPICanno.ilm10b2.hg19)      # NOT RUN
# groomed_data <- grooMethy(betas_clean6, Seq.GR = genomic_location_matrix)      # NOT RUN

# groomed_data <- grooMethy(betas_clean6)
# write_rds(groomed_data, "intermediary data/groomed_data_07262023.rds")
...
# beta %>% as_tibble() %>% select(all_of(outlier_sample)) %>% 
#   # pivot_longer(cols = - Index) %>% 
#   ggplot(aes(x= `200360610122_R02C01`))+
#   geom_density()+
#   xlim(0,1)+
#   theme(legend.position = "none")
```
...
#                  work.dir = here::here())
# write_rds(remp_res_ERV, "intermediary data/remp_results_ERV_07262023.rds")
remp_res_ERV <- read_rds(paste0(here::here(), "/intermediary data/remp_results_ERV_07262023.rds"))
```
...
```{r before trimming}
remp_res_ERV_temp <- rempTrim(remp_res_ERV,threshold=1.7,missingRate=0.1)
mat <- rempB(remp_res_ERV_temp)
```

```{r}
mat1 <- mat %>% as_tibble() %>%
  cbind(L1 = remp_res_ERV_temp@rowRanges@elementMetadata$RE.Index, .)
...
```{r}
remp_res_ERV_temp <- rempTrim(remp_res_ERV,threshold=1.7,missingRate=0.15)
mat <- rempB(remp_res_ERV_temp)
```
...
```{r}
remp_res_ERV_temp <- rempTrim(remp_res_ERV,threshold=1.7,missingRate=0.20)
mat <- rempB(remp_res_ERV_temp)
```
...
```{r}
remp_res_ERV_temp <- rempTrim(remp_res_ERV,threshold=1.7,missingRate=0.25)
mat <- rempB(remp_res_ERV_temp)
```
...
# write_rds(remp_res_ERV, "intermediary data/remp_res_ERV_aggregate_07262023.rds")
remp_res_ERV <- read_rds(paste0(here::here(), "/intermediary data/remp_res_ERV_aggregate_07262023.rds"))
details(remp_res_ERV)
...
# write_rds(remp_res_ERV, "intermediary data/remp_res_ERV_annotation_07262023.rds")
...
map_df %>% select(all_of(outlier_sample)) %>% 
  # pivot_longer(cols = - Index) %>% 
...
map_df %>% select(all_of(outlier_sample)) %>% 
  summarise_all(list(percent_missing_for_X200360610122_R02C01 = ~sum(is.na(.))/length(.) * 100
                                    )
                               )
...
print(paste0("Missingness rate for the overall data is ",
             ...
             "%"))
```
Let's take a look at the 4 patient with more than 20% missingness
```{r}
map_df %>% select(X200503840077_R05C01, X200503840106_R08C01, X200503840042_R02C01,
                  X203717920012_R04C01) %>% 
